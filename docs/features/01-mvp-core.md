...
This feature plan expands `Feature 01. MVP Core` from the main backlog into executable stages. It should be completed before moving to chess engine play, early position analysis, time controls, online invite games, or PGN history polish.
...
- the implementation is ready for Feature 02 computer replies and early position analysis without changing the core game model.

## Stop Point

Stop Feature 01 when a signed-in user can create or open a basic game screen, make legal moves through the server-backed game service, see the board and SAN move list update, and persist the resulting position and moves in the database.

Do not start these items in Feature 01:
...
After Feature 01, prioritize the smallest useful engine slice before broader online play: a minimal `EngineService`, computer reply moves, and a first analysis screen that accepts or sets up a FEN position and returns a bounded evaluation.
...
Local user synchronization path: `getCurrentUserIdentity()` returns a trimmed Clerk identity for server code. Stage 05 should upsert the local `User`, `UserAuthIdentity`, and default `Rating` from that helper before game creation. Game and rating records must reference internal `User.id` values, while Clerk user IDs stay inside `UserAuthIdentity`.
...
- [ ] Create two participants for the game, even when Feature 01 has no online opponent.
...
- [ ] Choose the Feature 01 server boundary: route handlers, server actions, or a small internal API facade.
...
## Stage 11. Persistence Verification
...
## Stage 12. Documentation Updates

- [ ] Update `docs/README.md` if the active next task changes.
- [ ] Update `docs/backlog.md` when Feature 01 checklist items are completed.
...
Start Stage 07 by adding `chess.js` and implementing the first `GameService` create-game and legal-move operations.
